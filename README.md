...
- `save/` contains a versioned world/player snapshot format, automatically saved on pause/disposal and restored on next launch.
...
## Current status
The voxel sandbox loop remains a prototype: seeded terrain, first-person movement/jump, mining/placing, inventory, and touch controls. The code has been reorganized into subsystem boundaries, but device-specific stability still requires testing on the target phone. CI build/test results—not a successful local/device run—are the verification source for this branch.
